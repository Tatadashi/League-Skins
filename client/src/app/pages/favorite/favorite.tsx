...
import type { Skin } from "../collection/collection";
import type { Dispatch, SetStateAction } from "react";

export default function Favorite() {
  //need type states so that filtered fn doesn't error
  const [favorited, setFavorited]: [Skin[], Dispatch<SetStateAction<Skin[]>>] =
    useState([] as Skin[]);
  useEffect(() => {
    setFavorited(JSON.parse(String(localStorage.getItem("favorites"))));
  }, []);

  //filter by name (caseinsensitive)
  function filterSkins(query: string) {
    const filtered: Skin[] = [];
    favorited.forEach((skin: Skin) => {
      if (
